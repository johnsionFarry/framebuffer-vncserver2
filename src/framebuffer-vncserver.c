...
// Dynamic values based on bit depth
...
static void init_fb(void)
{
    ...
    // Initialize dynamic pixel format values based on bit depth
    switch (var_scrinfo.bits_per_pixel)
    {
    case 1:
        BITS_PER_SAMPLE = 1;
        SAMPLES_PER_PIXEL = 1;
...
static void init_fb_server(int argc, char **argv, rfbBool enable_touch, rfbBool enable_mouse)
{
    ...
    /* Use dynamic values based on actual bit depth */
...
static void update_screen(void)
{
    ...
    {
        // Handle rotation for all bit depths
        uint8_t *f = (uint8_t *)fbmmap; /* -> framebuffer         */
        uint8_t *c = (uint8_t *)fbbuf;  /* -> compare framebuffer */
        uint8_t *r = (uint8_t *)vncbuf; /* -> remote framebuffer  */
        ...
        if (memcmp(fbmmap, fbbuf, frame_size) != 0)
        {
            ...
            for (y = 0; y < (int)fb_yres; y++)
            {
                ...
                for (x = 0; x < (int)fb_xres; x++)
                {
                    ...
                    // Read comparison pixel
                    uint32_t comp_pixel = 0;
                    if (bytespp == 4)
                    {
                        comp_pixel = *(uint32_t *)c;
                    }
                    else if (bytespp == 2)
                    {
                        comp_pixel = *(uint16_t *)c;
                    }
                    else if (bytespp == 1)
                    {
                        comp_pixel = *c;
                    ...
                    if (pixel != comp_pixel)
                    {
                        ...
                        // Write back to comparison buffer
                        if (bytespp == 4)
                        {
                            *(uint32_t *)c = pixel;
                        }
                        ...
                        // Calculate remote buffer offset
                        uint8_t *remote_pixel = r + (y2 * server->width + x2) * bytespp;

                        // Process pixel based on bit depth
                        if (bytespp == 4)
                        {
                            uint32_t rem = PIXEL_FB_TO_RFB(pixel, varblock.r_offset, varblock.g_offset, varblock.b_offset);
                            *(uint32_t *)remote_pixel = rem;
                        }
                        else if (bytespp == 2)
                        {
                            uint16_t rem = PIXEL_FB_TO_RFB(pixel, varblock.r_offset, varblock.g_offset, varblock.b_offset);
                            *(uint16_t *)remote_pixel = rem;
                        }
                        else if (bytespp == 1)
                        {
                            *remote_pixel = pixel;
